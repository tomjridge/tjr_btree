...
module Internal = struct
  ...
  let make_map_ops ~monad_ops ~pre_btree_ops ~root_ops =
    let ( >>= ) = monad_ops.bind in
    let return = monad_ops.return in
    let Isa_btree_intf.{ find; insert; delete; insert_many; insert_all; leaf_stream_ops; leaf_ops; _ } = pre_btree_ops in
...
('b, 'c, 'a) Map_ops_etc_type.map_ops_etc *
('b, 'c, 'd, 'g, 'a) leaf_stream_ops
 = make_map_ops
