               ...
               (fun a ->
                 (match a
                   with Frame.Node_frame (ks, rs) ->
                     let (stka, ra) =
                       Tree_stack.add_new_stack_frame Params.compare_k k
                         (ks, rs) stk
                       in
                     F_down (r0, (k, (ra, stka)))
       ...
       Tree.wellformed_tree Params.equal_k Params.constants
         ...
         List.equal_lista
           ...
           (Util.rev_apply (Util.rev_apply t0 Tree.tree_to_kvs)
             (Key_value.kvs_delete Params.equal_k Params.compare_k k))
  ...
  t0 k str u =
    Util.assert_truea
      (let n = Tree.height t0 in
       ...
       let check_wf =
         (fun ms ->
           Tree.wellformed_tree Params.equal_k Params.constants ms
             Params.compare_k)
         in
       let check_focus =
         (fun foa ->
        ...
        let f =
          (match Arith.equal_nat p_sz Arith.zero_nat
            ...
            | false ->
              (match
                Arith.less_nat p_sz
                  (Util.rev_apply Params.constants Prelude.min_node_keys)
                with true ->
                  Monad.return
...
let rec steal_or_merge
  right leaf mk_c c p_k s =
    ...
    (fun s_1 ->
      (match
        Arith.less_nat
          (if leaf then Util.rev_apply Params.constants Prelude.min_leaf_size
            else Util.rev_apply Params.constants Prelude.min_node_keys)
                      ...
                      with true ->
                        ...
                        (match
                          Arith.less_nat (List.size_list kvsa)
                            (Util.rev_apply Params.constants
          ...
          Util.rev_apply (Params.store_free (r0 :: Tree_stack.r_stk_to_rs stk))
            (Monad.bind
              (fun _ ->
                let kvsa =
                  Util.rev_apply kvs
                    (Key_value.kvs_insert Params.equal_k Params.compare_k
                      (k, v))
                  in
                let fo =
                  (match
          ...
          (fun (_, (ks2, rs2)) ->
            (match fo
              ...
              | I2 ((r1, (k, r2)), kvs0) ->
                ...
                (match
                  Arith.less_eq_nat (List.size_list ks)
                    (Util.rev_apply Params.constants Prelude.max_node_keys)
                  with true ->
                    Util.rev_apply
                      (Util.rev_apply (Frame.Node_frame (ks, rs))
                        Params.store_alloc)
                  ...
                  | false ->
                    let (ks_rs1, (ka, ks_rs2)) =
                      Key_value.split_node Params.constants (ks, rs) in
                    Util.rev_apply
                      (Util.rev_apply (Frame.Node_frame ks_rs1)
                        Params.store_alloc)
                      (Monad.bind
...
let rec split_leaf
  kvs = let n1 = List.size_list kvs in
        ...
        let delta = Util.rev_apply Params.constants Prelude.min_leaf_size in
        let n1a = Arith.minus_nat n1 delta in
        let n2a = Arith.plus_nat n2 delta in
        let deltaa =
          Arith.minus_nat n1a
            (Util.rev_apply Params.constants Prelude.max_leaf_size)
          in
        ...
        (match
          ...
          | false ->
            (match newb with [] -> None
              | (k, v) :: newc ->
                (match
                  Key_value.check_keys Params.equal_k Params.compare_k None
                    (Set.insert Params.equal_k k Set.bot_set) u
                  with true ->
                    Some (Key_value.kvs_insert Params.equal_k Params.compare_k
                            (k, v) acc,
                           newc)
                  | false -> None))))
      in
...
module Leaf_stream : sig
  val lss_step :
    (Params.k, Params.v, Params.page_ref) Pre_params.ls_state ->
      (Params.k, Params.v, Params.page_ref) Pre_params.ls_state Params.mm
  val mk_ls_state :
    Params.page_ref -> (Params.k, Params.v, Params.page_ref) Pre_params.ls_state
  val dest_LS_leaf :
    (Params.k, Params.v, Params.page_ref) Pre_params.ls_state ->
      ((Params.k * Params.v) list) option
  val lss_is_finished :
    (Params.k, Params.v, Params.page_ref) Pre_params.ls_state -> bool
end = struct
...
let rec lss_step
  lss = (match lss with Pre_params.LS_down a -> step_down a
          | Pre_params.LS_leaf x -> Monad.return (step_leaf x)
          | Pre_params.LS_up x -> Monad.return (step_up x));;

let rec mk_ls_state r = Pre_params.LS_down (r, []);;

let rec dest_LS_leaf
  x = (match x with Pre_params.LS_down _ -> None
        | Pre_params.LS_leaf (kvs, _) -> Some kvs
        | Pre_params.LS_up _ -> None);;

let rec lss_is_finished
  lss = (match lss with Pre_params.LS_down _ -> false
          | Pre_params.LS_leaf _ -> false | Pre_params.LS_up [] -> true
