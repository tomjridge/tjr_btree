...
module Store_backed_by_disk = 
  functor (Disk: DISK with type 'a m = 'a World.m) -> struct
    module Disk = Disk

    include M
    module Page = Disk.BLK
    ...
    type t = state World.r

    let get_disk: t -> Disk.t m = (fun t ->
        get t |> bind (fun s -> return s.disk))

    let get_free: t -> int m = (fun t ->
        get t |> bind (fun s ->
            return s.free))

    let set_free: t -> int -> unit m = (fun t i ->
        get t |> bind (fun s ->
            set t {s with free=i}))

    (* returns a fresh block identifier *)
    (* alloc without write; free block can then be used to write data
       outside the btree *)
    let alloc_block: t -> Page.r m = (fun t ->
        get_free t |> bind (fun i -> set_free t (i+1) |> bind (fun _ -> return i)))

    let alloc: t -> Page.t -> Page.r m = (fun t p ->
        alloc_block t |> bind (fun r -> 
            get_disk t |> bind (fun d ->
                Disk.write d r p |> bind (fun () -> return r))))

    let free: t -> Page.r list -> unit m = (fun t ps -> return ())  
    (* FIXME do better *)
...
(* check result has right type *)
module X_ = functor (Disk: DISK with type 'a m = 'a World.m) -> struct
  module X : STORE = Store_backed_by_disk(Disk)
end
